    ...
    <div class="body-wrapper">
      ...
      <div class="container-fluid">
        <div class="container-fluid">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title fw-semibold mb-4 justify-content-center"><strong>Project List</strong></h5>
              <div class="row mt-3">
                <div class="col-4">
                  <div class="card alert-success" style="width: 100%;">
                    <div class="card-body">
                      <h5 class="card-title"><strong>Total Project</strong></h5>
                      <?php 
                        $sqlCount = "SELECT count(id) as `jumlah_project` FROM re_project_list";
                        $executeCount = mysqli_query($conn, $sqlCount);
                        $dataCount = mysqli_fetch_assoc($executeCount);
                      ?>
                      <p class="card-text"><?php echo $dataCount['jumlah_project']; ?></p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="card-upload">
                <h3 class="mt-3">Upload Project</h3>
                <hr/>
                <form class="mt-2" action="backend/action_push_event.php" method="POST" enctype="multipart/form-data">
                  <div class="formbold-input-flex">
                    ...
                    <div>
                      <label for="githubLink" class="formbold-form-label"> Github Link </label>
                      <input type="text" name="githubLink" id="githubLink" placeholder="Github Link"
                        class="formbold-form-input" />
                    </div>
                  </div>
                  <div class="formbold-input">
                    <div>
                      ...
                      <textarea name="description" id="description" placeholder="Enter your description" class="formbold-form-input"
                        style="height:200px;"></textarea>
                    </div>
                  </div>
                  <div class="formbold-input mt-2">
                    <div>
                      <label for="coverProject" class="formbold-form-label"> Cover Project </label>
                      <input type="file" name="coverProject" id="coverProject" placeholder="Cooper"
                        class="formbold-form-input" />
                    </div>
                  </div>
                  <div class="formbold-mb-5">
                    <label for="qusOne" class="formbold-form-label">
                      Yeay we are ready for your code, masters !
                    </label>
                    <button class="formbold-btn" name="submitProject">Submit</button>
                </form>
              </div>
              <div class="row mt-5">
                <table id="example" class="table table-striped table-bordered mt-4" style="width:100%">
                  ...
                  $data = selectSqlDataProject();
                  for($i = 0 ; $i<count($data); $i++){
                  ?>
                    <tr>
                      <td><?php echo $i+1; ?></td>
                      <td><?php echo $data[$i]['project_name']; ?></td>
                      <td><?php echo $data[$i]['cover_project']; ?></td>
                      <td><?php echo $data[$i]['github_link']; ?></td>
                      <td><?php echo $data[$i]['created_date']; ?></td>
                      <td>
                        <form action="backend/action_push_event.php" method="POST" enctype="multipart/form-data">
                          <input type="text" name="idProject" class="formbold-form-input" value="<?php echo $data[$i]['id']; ?>" hidden/>
                          <input type="text" name="coverProject" class="formbold-form-input" value="<?php echo $data[$i]['cover_project']; ?>" hidden/>
                          <button type="submit" name="deleteProject" class="btn btn-danger">Hapus</button>
                        </form>
                      </td>
                    </tr>
                    <?php  
